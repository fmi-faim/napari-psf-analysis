...
def analyze_bead(
    ...
    zyx_popt, zyx_perr = fit_3d_yx(img=bead, spacing=spacing)
...
def localize_beads(
    ...
    beads = []
    offsets = []
    margins = np.array(patch_size) / np.array(spacing) / 2
    for p in points:
        if np.all(p > margins) and np.all(p < (np.array(img.shape) - margins)):
            ...
            z_slice = _create_slice(closest_roi[0] + z_search_slice.start, margins[0])
            y_slice = _create_slice(closest_roi[1] + y_search_slice.start, margins[1])
            x_slice = _create_slice(closest_roi[2] + x_search_slice.start, margins[2])
